...
// Uniform jitter has a hard ceiling; a hang rate is the only heavy tail the sim can produce.
const TIMEOUT_RATE = num("MEASURE_TIMEOUT_RATE", 0);
...
const MODES = (process.env.MEASURE_MODES ?? "parallel,serial").split(",");
...
const ARMS = [
  ...
  ...MODES.map((mode) => [`fanout-${mode.slice(0, 3)}`, `/fanout?n=${FANOUT_N}&mode=${mode}`]),
];
